...
void AddPlanetEffects(INT16 modelindex, INT32 randSeed, SINT32 efScale, SINT32 *effects, INT8 *worldPorts, SINT32 *numWorldPorts)
{
	INT32 randSeed2;
...
// completely deactivated, because of stack corruptions in the vicinity of sysObjects (needs an evaluation with valgrind)
void GenSystemBasicType(INT32 id, sysType_t *type, INT32 *techLevel, government_t *govType, INT8 *allegiance)
{
// to deactivate this has consequences: start point four crashes right away
#ifdef _GENERATE_POPULATED_SYSTEMS_AT_RANDOM_
...
void GetSupplyFromEffects(INT8 *supply, SINT32 effects[EF_MAX], government_t government)
{
...
void FlagIllegalGoods(INT8 *supply, sysType_t *type, government_t government)
{
	INT8 i;
...
void Override_F869(INT32 id, INT32 *overStr, INT32 *descStr, INT32 *techLevel, INT32 *pirates, INT32 *policeLevel, INT32 *traders, INT32 *miners, INT32 *government)
{
	sysType_t type;
...
void Override_F870(INT32 id, INT8 **stockFlags, INT32 *population, INT32 *danger, INT32 *d4)
{
	...
	*population = 2 + 8 * (type.development / 256.0);
	...
	FlagIllegalGoods(*stockFlags, type.effects, govType);
...
void Override_F871(INT32 id, INT32 *d1, INT32 *d2, INT32 *milActivity, INT32 *allegiance, INT32 *milRankSub, INT32 *milRankBase)
{
	INT32 overStr, descStr, d3, pirates, techLevel, traders, d7, government;
	sysType_t type;
	government_t govType;
