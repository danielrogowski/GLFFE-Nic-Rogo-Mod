...
// OpenGL 1.1+ implementation by Kasak Sergey ( viper_craft@rambler.ru )
// Warning! works only with old cards! New cards ( PS2.0+ ) should use original version!
...
bool Nic_glColorTableEXT(GLenum target, GLenum internalFormat, GLsizei width, GLenum format, GLenum type, const void* data)
{
	if (glColorTableEXT) 
	{
		...
		//glColorTableEXT(textureTarget, GL_RGB, 256, GL_RGB, GL_UNSIGNED_BYTE, mapP);
...
static UCHAR		pPalTexture[256 * 6];
...
{
	// set up trig luts
    static const float coef = 2.0f * fPi / 256.0f;

	for( int count = 0; count < 256; ++count )
    {
		sinLUT[count] = sin(coef * (float)count);
		cosLUT[count] = cos(coef * (float)count);
	}
	...
	if( !g_CfgFullscreen ) 
        pScreen = SDL_SetVideoMode (g_nWidth, g_nHeight, 0, SDL_OPENGL);
	else 
        pScreen = SDL_SetVideoMode (g_nWidth, g_nHeight, 0, SDL_OPENGL | SDL_FULLSCREEN);
    ...
    memset(pBitmaps, 0, sizeof pBitmaps);
    memset(bReplacedBitmap, 0, sizeof bReplacedBitmap);
...
/*    if
    ...
    {
        fprintf(stderr, "nessesary extensions found, proceeding...\n");
    }
    else
    {
        fprintf(stderr, "nessesary extensions NOT found, aborting.\n");
        return EXIT_FAILURE;
    }
*/
    /*
    ...
    */
	...
	SDL_Surface* cabin = LoadBMP("96.jpg");
	if( cabin != NULL ) 
    {
	...
	// load crosshair
/*	SDL_Surface* crosshair = LoadBMP("crosshair.png");
	if( crosshair != NULL ) 
    {
	//	glActiveTexture(GL_TEXTURE1);
	//	glEnable(GL_TEXTURE_2D);
	...
	/*
	// load drift marker
	SDL_Surface* driftmarker = LoadBMP("driftmarker.png");
	if( driftmarker != NULL ) 
    {
		//glActiveTexture(GL_TEXTURE1);
		//glEnable(GL_TEXTURE_2D);
...
*/
	// load title
	SDL_Surface* title = LoadBMP("title.jpg");
	if( title != NULL ) 
    {
		//glActiveTexture(GL_TEXTURE1);
		//glEnable(GL_TEXTURE_2D);
	...
	if( g_CfgReplaceConsole ) 
    {
        ...
        {
			char fname[128];
			_snprintf(fname, sizeof(fname), "console%d.png", i);
...
{
	USHORT color    = (USHORT)(col & 0xFFFF);
...
extern "C" void SDLDrawTriangleInternal
(
    SHORT   p1[2], 
    SHORT   p2[2], 
    SHORT   p3[2], 
...
extern "C" void SDLDrawQuadInternal
(
    SHORT   p1[2], 
    SHORT   p2[2], 
    SHORT   p3[2], 
...
{	
	...
	if ( !MemCmp(SDLTextureColors_old[texnum], SDLTextureColors, 32, 1) )
	{
		if ( !Nic_glColorTableEXT(GL_TEXTURE_2D, GL_RGBA, 32, GL_RGBA, GL_UNSIGNED_BYTE, lutBuffer) )
			glTexImage2D	(GL_TEXTURE_2D, 0, 4, sizex, sizey, 0, GL_COLOR_INDEX, GL_UNSIGNED_BYTE, texels);
		memcpy(SDLTextureColors_old[texnum], SDLTextureColors, 32*2);
	}
...
/*	if ( Tex[texnum] > 0 )
	{
		if ( !Nic_glColorTableEXT(GL_TEXTURE_2D, GL_RGBA, 256, GL_RGBA, GL_UNSIGNED_BYTE, lutBuffer) )
			glTexImage2D	(GL_TEXTURE_2D, 0, 4, sizex, sizey, 0, GL_COLOR_INDEX, GL_UNSIGNED_BYTE, texels);
		Tex[texnum]--;
	}
*/		
/*
	...
	glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, sizex, sizey, GL_COLOR_INDEX, GL_UNSIGNED_BYTE, texels);
...
*/
//	glTexImage2D(texnum, 0, GL_RGBA, sizex, sizey, 0, format, pixelType, surface->pixels
//	glTexImage2D(GL_TEXTURE_2D, 0, 2, 256, 256, 0, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, pNewBuffer);
...
extern "C" void SDLDrawTexTriangleInternal
( 
    long    *texels, 
    SHORT   p1[2], 
    SHORT   p2[2], 
...
extern "C" void SDLDrawTexQuadInternal
(
    long    *texels, 
    SHORT   p1[2], 
    SHORT   p2[2], 
...
{	
	...
	if( pBitmaps[bmpnum] == 0 ) 
    {
		...
		if ( !Nic_glColorTableEXT(GL_TEXTURE_2D, GL_RGBA, 256, GL_RGBA, GL_UNSIGNED_BYTE, pPalTexture) )
...
void DoBlitIcon( int bmpnum, float gaugeFraction = 0.0f, int pylon = -1 ) 
{
    ...
    else 
    {
		...
		//glUniform1i ( textureLocation, 0 );
		//GLint colorLocation = glGetUniformLocation (sp_char, "color");
		SetColorFrom12bit( bmpLUT[bmpnum].nMode );
	}
